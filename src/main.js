...
const encodeToBase64 = (...args) => {
  try {
    ...
    console.log(`Combined String for Base64: ${string}`)
...
const encodeToHex = (...args) => {
  try {
    const string = args.join(':')
    console.log(`Combined String for Hex: ${string}`)

    const encoded = Buffer.from(string, 'utf-8').toString('hex')
    console.log(`Hex Encoded Result: ${encoded}`)
    return encoded
  } catch (error) {
    console.error('Error during Hex encoding:', error)
    return null
  }
}
...
const decodeFromHex = (hexString) => {
  try {
    ...
    console.log(`Decoded Hex Result: ${decoded}`)
    return decoded
  } catch (error) {
    console.error('Error during Hex decoding:', error)
    return null
  }
}
...
const safeDecodeFromBase64 = (base64String) => {
  ...
  try {
    ...
    const decoded = Buffer.from(base64String, 'base64').toString('utf-8')
    console.log(`Successfully decoded from Base64: ${decoded}`)
    return decoded
  } catch (error) {
...
const safeDecodeFromHex = (hexString) => {
  ...
  try {
    ...
    const decoded = Buffer.from(hexString, 'hex').toString('utf-8')
    console.log(`Successfully decoded from Hex: ${decoded}`)
    return decoded
  } catch (error) {
